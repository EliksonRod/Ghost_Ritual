...
public class EnemyScript : MonoBehaviour
{
    ...
    public BevaviorState targetState;
    // Defines what monster is currently doing
    public enum BevaviorState
    {
        Wandering,
        Idle,
        Chasing,
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(transform.position + rayCastOffset, direction, out hit, sightDistance))
        {
            ...
            if (hit.collider.gameObject.tag == "Player" && IgnorePlayer == false)
            {
                ...
                //wandering = false;
                targetState = BevaviorState.Chasing;
    ...
    {
        targetState = BevaviorState.Wandering;
        //chasing = false;
        StopCoroutine("chaseRoutine");
        currentDest = PatrolDestinations[Random.Range(0, PatrolDestinations.Count)];
    }
    ...
    void MonsterStateHandler()
    {
        switch (targetState)
        {
            case BevaviorState.Wandering:
                WanderAround();
                break;
            case BevaviorState.Idle:
                break;
            case BevaviorState.Chasing:
                ChasePlayer();
                break;
        }
    ...
    void WanderAround()
    {
        ...
        if (monsterAI.remainingDistance <= monsterAI.stoppingDistance + 0.1f)
        {
            ...
            targetState = BevaviorState.Idle;
        }
    }
